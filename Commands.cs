    ...
    class Commands : Script
    {
        ...
        public static void CMD_ammo(ExtPlayer player, int ID, int type, int amount = 1)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) მიცა ტყვიები ({type}|{amount}) {target.Name} ({target.Value})");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_giveredsall(ExtPlayer player, int amount)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "თქვენ შეგიძლიათ გაუშვათ მინიმუმ 1 ST და მაქსიმუმ 500 ST", 3000);
                    return;
                }
                ...
                foreach (ExtPlayer foreachPlayer in NeptuneEvo.Character.Repository.GetPlayers())
                {
                    ...
                    UpdateData.RedBucks(foreachPlayer, correctValue, msg: "გაგზავნა ST");
                ...
                NAPI.Chat.SendChatMessageToAll($"{CommandsAccess.AdminPrefixChat}{player.Name.Replace('_', ' ')} გაიცემა ყველა მოთამაშეზე {amount} RedBucks.");
            ...
            try
            {
                ...
                if (targetCharacterData.BizIDs.Count > 0)
                {
                    try
                    {
                        ...
                        Trigger.SendChatMessage(player, $"მოთამაშე ფლობს ბიზნესს {BusinessManager.BusinessTypeNames[biz.Type]} (ID:{biz.ID})");
                    }
                ...
                if (house != null)
                {
                    ...
                    {
                        Trigger.SendChatMessage(player, $"მოთამაშეს აქვს უძრავი ქონება (ID{house.ID}) ღირებულება ${house.Price} კლასი '{HouseManager.HouseTypeList[house.Type].Name}'");
                    ...
                    {
                        Trigger.SendChatMessage(player, $"მოთამაშე დასახლებულია სახლში (ID{house.ID}) к {house.Owner} ღირებულება ${house.Price} კლასი '{HouseManager.HouseTypeList[house.Type].Name}'");
        ...
        public static void CMD_setDim(ExtPlayer player, int id, int dim)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value})  Dimension({dim}) {target.Name} ({target.Value})");
        ...
        public static void CMD_takeOffBusiness(ExtPlayer admin, int bizid, bool byaclear = false)
        {
            try
            {
                ...
                if (characterData == null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"მოთამაშე ვერ მოიძებნა", 3000);
                        return;
                    }
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            if (character == null)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"ადამიანი ვერ მოიძებნა", 3000);
                                return;
                            }
        ...
        public static void CMD_MoneyMultiplier(ExtPlayer player, int multi)
        {
            try
            {
                ...
                if (multi < 1 || multi > 5)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"შესაძლებელია მხოლოდ 1-დან 5-მდე დაყენება", 3000);
                    return;
                }
                ...
                */
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"MoneyMultiplier შეიცვალა {multi}", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_offlineDelFraction(ExtPlayer player, string name)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენ გაკიკეთ {name} ფრაქციდან", 3000);
                Manager.RemoveFractionMemberData(name);
                GameLog.Admin($"{player.Name}", $"delfrac", $"{name}");
                Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) გაკიკული ფრაქციდან {name}");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"თქვენ ჩახსენით ფრაქციდან {name}", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_removeObject(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"შემდეგი მცდელობა  იქნება ბანი", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_unwarn(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"მოთამაშეს არ აქვს გაფთხილება", 3000);
                    return;
                }
                ...
                NAPI.Chat.SendChatMessageToAll("!{#DF5353}" + $"ადმინისტრატორმა {player.Name} მოხსნა გაფტხილება {target.Name}({target.Value}).");

                //Chars.Repository.PlayerStats(target);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენ მოხსენი გაფთხილება {target.Name}, დარჩენილი აქ {targetCharacterData.Warns} გაფთხილება", 3000);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"თქვენ მოგხსნეს გაფთხილება, დარჩენილია {targetCharacterData.Warns} გაფთხილება", 3000);
        ...
        public static void CMD_offunwarn(ExtPlayer player, string targetName)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "მოთამაშე იყო ონლაინ რეჟიმში, ამიტომ offunwarn შეიცვალა unwarn-ით", 3000);
                    return;
                }
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        if (warninfo == null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"შეცდომა Warn'a-ს წაშლისას. უთხარი დეველოპერს", 3000);
                            return;
                        }
                        if (warns <= 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"მოთამაშეს არ აქვს ვარნი", 3000);
                            return;
                        }
                        warns--;
                        ...
                        Trigger.SetMainTask(() =>
                        {
                            NAPI.Chat.SendChatMessageToAll("!{#DF5353}" + $"ადმინისტრატორმა {sessionData.Name} მოხსნა გაფთხილება {targetName}  offline");
                            GameLog.Admin($"{sessionData.Name}", $"offUnwarn", $"{targetName}");
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენ მოხსენით გაფთხილება {targetName}, მას დარჩა {warns} გაფთხილება", 3000);
                        });
        ...
        public static void CMD_respawnCars(ExtPlayer player)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) sent all cars to spawn ({countToSpawn} ცალი.)");
                ...
                if (vehicleLocalData != null)
                {
                    switch (vehicleLocalData.Access)
                    {
                        ...
                        case VehicleAccess.DeliveryBike:
                            ...
                            return;
                        default:
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამ ტრანსპორტის სპაუნზე გაგზავნა შეუძლებელია.", 3000);
                            return;
                    }
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) სპაუნზე გაუშვა მანქანა ({vehicle.Value})");
            ...
            try
            {
                ...
                foreach (ExtVehicle veh in vehs)
                {
                    ...
                    if (vehicleLocalData != null)
                    {
                        ...
                        Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value})  სპაუნზე გაუშვა მანქანა ({veh.Value})");
        ...
        */
        ...
        public static void CMD_GetMyPromo(ExtPlayer player)
        {
            try
            {
                ...
                if (pcdata == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "თქვენს პერსონაჟთან დაკავშირებული პრომო კოდი ვერ მოიძებნა", 3000);
                    return;
                }
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენი პრომო კოდი გამოყენებულია {pcdata.UsedTimes} ჯერ.", 10000);
                switch (pcdata.RewardVipLvl)
                {
                    case 1:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"ბონუსი: Silver VIP   {pcdata.RewardVipDays} დღე, {pcdata.RewardMoney}$ და {pcdata.RewardItems.Count} ნივთები.", 10000);
                        break;
                    case 2:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"ბონუსი: Gold VIP на {pcdata.RewardVipDays} დღე, {pcdata.RewardMoney}$ და {pcdata.RewardItems.Count} ნივთები.", 10000);
                        break;
                    case 3:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"ბონუსი: Platinum VIP на {pcdata.RewardVipDays} დღე, {pcdata.RewardMoney}$ და {pcdata.RewardItems.Count} ნივთები.", 10000);
                        break;
                    case 4:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"ბონუსი: Diamond VIP на {pcdata.RewardVipDays} დღე, {pcdata.RewardMoney}$ და {pcdata.RewardItems.Count} ნივთები.", 10000);
                        break;
                    default:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"ბონუსი: {pcdata.RewardMoney}$ და {pcdata.RewardItems.Count} ნივთები.", 10000);
                        break;
                }
                if (pcdata.DonatePercent != 0)
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"სხვა რაღაცებთან ერთად, თქვენ მიიღებთ {pcdata.DonatePercent * 100}% StreetCoin-ის სახით შემოწირულობიდან ანგარიშზე {pcdata.DonateLogin},როდესაც თქვენი პრომო კოდის მქონე მოთამაშე ავსებს თავის ანგარიშს (მაგალითად: 1% 100 ST მოთამაშის მიერ შევსებისას, თქვენ მიიღებთ 1ST-ს).", 15000);
                    if (pcdata.DonateReceivedByStreamer != 0) Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"საერთო ჯამში თქვენ მიღეთ  {pcdata.DonateReceivedByStreamer} StreeCoin სხვა მოთამაშებისგან.", 15000);
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "პრომო კოდები წარმატებით გადაიტვირთა!", 3000);
                    });
                });
            }
            catch (Exception e)
        ...
        public static void CMD_BonuscmdSync(ExtPlayer player)
        {
            try
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "ბონუს კოდები წარმატებით გადაიტვირთა!", 3000);
                });
            }
            catch (Exception e)
            {
        ...
        public static void CMD_EconomySync(ExtPlayer player)
        {
            try
            {
                ...
                Economy.Init();
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენ წარმატებით გადატვირთეთ სერვერის ეკონომიკა მონაცემთა ბაზიდან.", 5000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_FracCmdSync(ExtPlayer player, string cmd, byte fractionId, byte access)
        {
            try
            {
                ...
                if (Configs.FractionWeapons[fractionId].ContainsKey(cmd))
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენ დააყენეთ წვდომის ახალი დონე ({access}) იარაღისთვის / საბრძოლო მასალისთვის {cmd}", 3000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამ ფრაქციაში ასეთი წვდომა ვერ მოიძებნა", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_setTerritoryColor(ExtPlayer player, int gangid)
        {
            try
            {
                ...
                if (shapeData == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"თქვენ არ იმყოფებით არცერთ რეგიონში", 3000);
                    return;
                }
                int terrid = shapeData.Index;
                if (!GangsCapture.gangPointsColor.ContainsKey(gangid))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"ამ ID მქონე ბანდა არ არსებობს", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"ახლა ტერიტორიას №{terrid} ფლობს {Fractions.Manager.FractionNames[gangid]}", 3000);
        ...
        public static void CMD_checkwanted(ExtPlayer player, int id)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"ვარსკვლავები - {stars}", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_fixcar(ExtPlayer player)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) გაასწორა მანქანა ({vehicle.Value})");
        ...
        public static void CMD_fixcarID(ExtPlayer player, int id)
        {
            try
            {
                ...
                foreach (ExtVehicle veh in vehs)
                {
                    ...
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) შეაკეთა მანქანა ID ({veh.Value})");
        ...
        public static void CMD_showPlayerOffStats(ExtPlayer player, string Name)
        {
            try
            {
                ...
                if (target != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "ამ სახელის მქონე მოთამაშე იყო ონლაინ, ამიტომ offstats შეიცვალა სტატისტიკით", 3000);
                    return;
                }
                Trigger.SetTask(() =>
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"დასახელებული მოთამაშე {Name} სისტემაში არა", 3000);
                        return;
                    }
                    ...
                    charData.Add(weddingName.Length > 5 ? weddingName : "არა");//14
                    ...
                    Admin.AdminLog(characterData.AdminLVL, $"{sessionData.Name} ({sessionData.Value}) უყურებს ოფლაინ სტატისტიკას {Name}");
                });
        ...
        public static void CMD_showPlayerStats(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= 6)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.CantFindPlayerWithId), 3000);
                        Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{player.Name} ({player.Value}) შევეცადე გადახედო შენს სტატისტიკას (/stats)", 3000);
                        return;
                    }
                }
                ...
                Trigger.ClientEvent(player, "client.inventory.Open");
                sessionData.LookingStats = true;
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) სტატისტიკას უყურებს {target.Name} ({target.Value})");
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                foreach (ExtPlayer foreachPlayer in Main.AllAdminsOnline)
                {
                    ...
                    if (afkData.IsAfk)
                    {
                        ...
                        afkTimeInfo = $"[AFK {inAFK.Minutes + 1} წუთი]";
                    }

                    Trigger.SendChatMessage(player, $"[{foreachPlayer.Value}] {foreachPlayer.Name} - {foreachCharacterData.AdminLVL} {afkTimeInfo}");
                }
                Trigger.SendChatMessage(player, "=== ADMINS ONLINE ===");
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ამოწმებს ადმინისტრაციის სიას ქსელში");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_AllMedias(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "=== MEDIA ONLINE ===");
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ამოწმებს მედიის სიას ქსელში");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_Clear(ExtPlayer player, int id)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ამოიღო ძებნილი სია {target.Name} ({target.Value})");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"თქვენ წაშალეთ ძებნა {target.Name} ({target.Value})", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_Agl(ExtPlayer player, int id)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) გასცა ლიცენზიების ნაკრებ {target.Name} ({target.Value})");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"თქვენ გაეცით ლიცენზიების ნაკრები {target.Name} ({target.Value})", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_Clear Exception: {e.ToString()}");
        ...
        public static void CMD_additem(ExtPlayer player, string Name, int Value, string Data)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, $"~r~შენ გაგზავნე {Chars.Repository.ItemsInfo[ItemId].Name}!");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_nalog(ExtPlayer player)
        {
            try
            {
                ...
                if (toggled)
                    Trigger.SendChatMessage(player, $"გადასახადის ჩამოწერა შედის!");
                else
                    Trigger.SendChatMessage(player, $"გადასახადის ჩამოწერა!");
            }
            catch (Exception e)
            {
                Log.Write($"CMD_goadditem Exception: {e.ToString()}");
        ...
        public static void CMD_setprod(ExtPlayer player, int id)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "~r~თქვენ მთლიანად შეავსეთ ბიზნესის საწყობი!");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_changeHousePrice(ExtPlayer player, int newPrice)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"თქვენ უნდა დადგეთ სახლის მარკერზე", 3000);
                    return;
                }
        ...
        public static void CMD_changeStock(ExtPlayer player, int fractionId, string item, int amount, bool add = false)
        {
            try
            {
                ...
                if (fractionData == null)
                {
                    Trigger.SendChatMessage(player, "~r~ამ ფრაქციის საწყობი არ არის");
                    return;
                }
                switch (item)
                {
                    ...
                    case "money":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"არ არის საკმარისი უფლებები", 3000);
                            return;
                        }
                        ...
                        break;
                    default:
                        Trigger.SendChatMessage(player, "~r~mats - მასალები");
                        Trigger.SendChatMessage(player, "~r~drugs - ნარკოტიკები");
                        Trigger.SendChatMessage(player, "~r~medkits - პირველადი დახმარების ნაკრები");
                        Trigger.SendChatMessage(player, "~r~money - ფული");
                        return;
                }
                fractionData.UpdateLabel();
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "თქვენ წარმატებით შეცვალეთ საწყობი.", 1500);
        ...
        public static void CMD_tpCoord(ExtPlayer player, string cords)
        {
            try
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        player.SendChatMessage("შეცდომა! შეიყვანეთ სწორი კოორდინატები");
                        return;
                    }
                }
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ტელეპორტირებული კოორდინატებზე {resultCords[0]} {resultCords[1]} {resultCords[2]}");
            ...
            try
            {
                ...
                if (characterData.AdminLVL >= 6 || Admin.SendCreatorQueue.ContainsKey(target))
                {
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "მხოლოდ სხვა ადმინისტრატორს შეუძლია დაადასტუროს!", 3000);
                            return;
                        }
                        Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) დაადასტურა ადმინისტრატორისგან გარეგნობის შეცვლის მოთხოვნა {Admin.SendCreatorQueue[target].Item1}");
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "თქვენ დაადასტურეთ მოთხოვნა გარეგნობის შეცვლის შესახებ!", 3000);
                    }
                ...
                else
                {
                    ...
                    Trigger.SendToAdmins(2, $"{ChatColors.StrongOrange}[A] მოთხოვნა საწყისი {player.Name} ({player.Value}) გარეგნობის შესაცვლელად {target.Name}. მოქმედების დასადასტურებლად - შეიყვანეთ: /sendcreator {id}");
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    if (vehicleLocalData.Access == VehicleAccess.Personal)
                    {
                        ...
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "ავტომობილი ხელახლა გამოშვების შემდეგ იქნება ხელმისაწვდომი", 3000);
                    }
                }
            }
            catch (Exception e)
        ...
        public static void CMD_setVehiclePetrol(ExtPlayer player, int fuel)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "თქვენ შეგიძლიათ მხოლოდ 200 ლიტრამდე შეავსოთ მანქანა!", 3000);
                    return;
                }
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) შეავსო მანქანა ({player.Vehicle.Value},{fuel}л)");
                }
            }
            catch (Exception e)
            {
        ...
        */
        ...
        public static void CMD_changeNamebyId(ExtPlayer player, int id, string newName)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Admin.ChangeNameQueue.Remove(curient);
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამ პერსონაჟისთვის მეტსახელის შეცვლის ძველი მოთხოვნა წაიშალა.", 3000);
                    }
        ...
        public static void CMD_changeName(ExtPlayer player, string curient, string newName)
        {
            try
            {
                ...
                if (!newName.Contains('_') || newName.Contains(' '))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "არასწორად შეგყავთ სახელი, სიმბოლო აკლია!", 3000);
                    return;
                }
                if (Main.PlayerNames.Values.Contains(newName))
                {
                ...
                if (split[0].Length < 3 || split[1].Length < 3)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "არასწორად შეგყავთ სახელი!", 3000);
                    return;
                }
                Regex rg = new Regex(@"^[a-z]+$", RegexOptions.IgnoreCase);
                if (!rg.IsMatch(split[0]))
                ...
                if (!rg.IsMatch(split[1]))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "არასწორად შეგყავთ გვარი ", 3000);
                    return;
                }
                if (characterData.AdminLVL >= 6 || (Admin.ChangeNameQueue.ContainsKey(curient) && Admin.ChangeNameQueue[curient].Item1 == newName))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "თქვენ არ შეგიძლიათ ადმინისტრატორის მეტსახელის შეცვლა, ადმინი მაღალი დონისაა!", 3000);
                        return;
                    }
                    if (characterData.AdminLVL >= 6)
                    {
                        Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) სახელი შეიცვალა {curient} - {newName} offline");
                        GameLog.Admin($"{player.Name}", $"changeName({newName})", $"{curient}");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "ნიკი შეიცვალა", 3000);
                    }
                    else
                    {
                        if (!Admin.ChangeNameQueue.ContainsKey(curient)) return;
                ...
                else
                {
                    ...
                    Admin.ChangeNameQueue.Add(curient, (newName, player.Name, characterData.AdminLVL));
                    Trigger.SendToAdmins(2, $"{ChatColors.StrongOrange}[A] მოთხოვნა საწყისი {player.Name} ({player.Value}) სახელის შესაცვლელად {curient} -> {newName}. მოქმედების დასადასტურებლად - შეიყვანეთ: /setnameoff {curient} {newName}");
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_startMatWars(ExtPlayer player)
        {
            try
            {
                ...
                if (MatsWar.isWar)
                {
                    Trigger.SendChatMessage(player, "~r~ზღვის ბრძლა მინდინარეობსს");
                    return;
                }
                MatsWar.startWar();
                Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} გაუშვა" + (characterData.Gender ? "" : "а") + " ზღვის ბრძლა!");
        ...
        public static void CMD_Stopmatwars(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Trigger.SendChatMessage(player, "~r~ზღვის ბრძლა, უნდა იყოს გაშვებული.");
                    return;
                }
                MatsWar.endWar();
                Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} დამთავრდა" + (characterData.Gender ? "" : "а") + " ზღვის ბრძლა");
        ...
        public static void CMD_giveLvl(ExtPlayer player, int id, int lvl)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"თქვენ მიეცით მოთამაშე {target.Name} ({target.Value}) {lvl} დონე", 3000);
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) გასცა {target.Name} ({target.Value}) {lvl} დონე");
        ...
        public static void CMD_replaceHousePrices(ExtPlayer player, int type, int newPrice)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, $"~r~თქვენ წარმატებით დაადგინეთ სახლების ფასი - {newPrice}");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_deleteHouseOwner(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"თქვენ უნდა იყოთ ქონების მარკერზე", 3000);
                    return;
                }
                ...
                Trigger.SendChatMessage(player, $"~r~თქვენ წარმატებით წაშალეთ სახლის მფლობელი {house.ID}");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_deleteThisAdminCar(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) წაშალა ადმინმა მანქანა ({vehicle.Value})");
                        VehicleStreaming.DeleteVehicle(vehicle);
        ...
        public static void CMD_delMyCars(ExtPlayer player)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value})  წაშალა ადმინმა მანქანა");
        ...
        public static void ACMD_setfractun(ExtPlayer player, int cat = -1, int id = -1)
        {
            try
            {
                ...
                if (!player.IsInVehicle)
                {
                    Trigger.SendChatMessage(player, "თქვენ უნდა იყოთ იმ ფრაქციის მანქანაში, რომლის შეცვლაც გსურთ");
                    return;
                }
                var vehicle = (ExtVehicle)player.Vehicle;
                var vehicleLocalData = vehicle.GetVehicleLocalData();
                if (vehicleLocalData != null)
                {
                    if (vehicleLocalData.Access == VehicleAccess.Fraction)
                    {
                        ...
                        VehicleManager.FracApplyCustomization(vehicle, fractionData.Id);
                        Trigger.SendChatMessage(player, "თქვენ შეცვალეთ ამ მანქანის ტიუნინგი ფრაქციისთვის..");
                    }
                    else Trigger.SendChatMessage(player, "თქვენ უნდა იყოთ იმ ფრაქციის მანქანაში, რომლის შეცვლაც გსურთ");
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    if (vehicleLocalData.Access == VehicleAccess.Fraction)
                    {
                        ...
                        if (isMessage) Trigger.SendChatMessage(player, "თქვენ შეცვალეთ ამ მანქანის მონაცემები ფრაქციისთვის.");
                    }
                    else if (isMessage) Trigger.SendChatMessage(player, "თქვენ უნდა იყოთ იმ ფრაქციის მანქანაში, რომლის შეცვლაც გსურთ.");
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_stopServer(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openDialog", "STOP_SERVER", $"დარწმუნებული ხართ, რომ გსურთ სერვერის შეჩერება?");
            }
            catch (Exception e)
            {
                Log.Write($"CMD_stopServer Exception: {e.ToString()}");
        ...
        public static void CMD_SaveServer(ExtPlayer player)
        {
            try
            {
                ...
                Admin.SaveServer();
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "სერვერი შენახულია", 1000);
        ...
        public static void CMD_inv(ExtPlayer player)
        {
            try
            {
                ...
                if (!player.OutgoingSyncDisabled)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "ახლა მოთამაშეები და თაღლითები ვერ გხედავენ", 1000);
                }
                else
                {
                    player.OutgoingSyncDisabled = false;
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "ახლა ყველას შეუძლია დაგინახოს.", 1000);
                }
        ...
        public static void CMD_Spawn(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (targetCharacterData.InsideHouseID == -1 && target.Dimension == 0 && targetCharacterData.DemorganTime <= 0 && targetCharacterData.ArrestTime <= 0 && !targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath && targetSessionData.SitPos == -1)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "თქვენ გაგზავნეთ მოთამაშე ქვირითისთვის!", 3000);
                    ...
                    EventSys.SendCoolMsg(target, "ადმინისტრაციამ", $"{player.Name}", $"დაგატელეპორტირა სპაუნზე", "", 10000);
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) დაგატელეპორტირა სპაუნზე {target.Name} ({target.Value})");
                    GameLog.Admin($"{player.Name}", $"spawn", $"{target.Name}");
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამჟამად მიუწვდომელია.", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_adminHeal(ExtPlayer player)
        {
            try
            {
                ...
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) გააუმჯობესა მისი ჯანმრთელობა");
        ...
        public static void CMD_createVehicle(ExtPlayer player, string name, int a, int b)
        {
            try
            {
                ...
                else
                {
                    if (characterData.AdminLVL <= 5)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამ სახელწოდების მანქანა ვერ მოიძებნა", 3000);
                        return;
                    }
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) მანქანა შექმნა({name},{veh.Value})");
            ...
            try
            {
                ...
                if (count >= 1 && count <= 20)
                {
                    ...
                    if (vehHash != 0)
                    {
                        ...
                        Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) შექმნა {count} მანქანა({name})");
                        GameLog.Admin($"{player.Name}", $"vehsCreate({count} {name})", $"");
                    }
                    else
                    {
                        if (characterData.AdminLVL <= 5) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამ სახელწოდების მანქანა ვერ მოიძებნა", 3000);
                        else
                        {
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "თქვენ შეგიძლიათ შექმნათ 20-მდე მანქანა ერთდროულად", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void aclear(ExtPlayer player, string targetName, string reason)
        {
            try
            {
                ...
                if (!Main.PlayerNames.Values.Contains(targetName) || !Main.PlayerUUIDs.ContainsKey(targetName))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამ სახელის მქონე ადამიანი ვერ მოიძებნა", 3000);
                    return;
                }
                ExtPlayer target = (ExtPlayer)NAPI.Player.GetPlayerFromName(targetName);
                if (target.IsCharacterData())
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "თამაშში მყოფი პერსონაჟის გასუფთავება შეუძლებელია", 3000);
                    return;
                }

                if (Main.stringGlobalBlock.Any(c => reason.Contains(c)))
                {
                    Trigger.SendToAdmins(1, $"{ChatColors.Red}[A] {sessionData.Name} ({sessionData.Value}) სისტემამ ამოიღო დასჯის მიზეზით: {reason}");
                ...
                Trigger.SetTask(() =>
                {
                    try
                    {
                        ...
                        if (result != null && result.Rows.Count != 0)
                        {
                            ...
                            {
                                Trigger.SendToAdmins(3, "!{#FF0000}" + $"[A] {sessionData.Name} ({sessionData.Value}) ცდილობდა გარკვევას {targetName} (offline),რომელსაც აქვს უფრო მაღალი ადმინისტრატორის დონე.");
                                return;
                            }
                            ...
                            if (TBiz.Count >= 1 && TBiz[0] >= 1)
                            {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"თქვენ აიღეთ ბიზნესი {owner}", 3000);
                            }
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "მონაცემთა ბაზაში პერსონაჟის პოვნა შეუძლებელია", 3000);
                            return;
                        }
                        // CLEAR HOUSE
                        ...
                        if (result4 != null && result4.Rows.Count != 0)
                        {
                            ...
                            if (house != null)
                            {
                                if (FurnitureManager.HouseFurnitures.ContainsKey(hid))
                                {
                                    ...
                                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"სახლიდან წახვედი {targetName} ყველა ავეჯი", 3000);
                                }
                                house.ClearOwner();
                                Houses.Rieltagency.Repository.OnPayDay(new List<House>()
                                {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"შენ აიღე ქონება {targetName}", 3000);
                                //Chars.Repository.PlayerStats(player);
                            }
                        }
                        // CLEAR VEHICLES
                        ...
                        if (vehiclesNumber.Count > 0)
                        {
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"თქვენ აიღეთ {targetName} ყველა მანქანა.", 3000);
                        }
                        ...
                        Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{sessionData.Name} პერსონაჟის სრულად გადატვირთვა {targetName}. მიზეზი: {reason}");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენ აიღეთ ყველაფერი ადამიანისგან, ფული ხელიდან და საბანკო ანგარიში {targetName}", 3000);
        ...
        public static void CMD_ClearFraction(ExtPlayer player, byte fraction)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.Center, $"თქვენ დაიწყეთ ფრაქციის წმენდა {Manager.FractionNames[fraction]}!", 3000);
        ...
        public static void CMD_Carnumber(ExtPlayer player, string number)
        {
            try
            {
                ...
                if (number.Length > 8)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "სანომრე ნიშნის სიმბოლოების რაოდენობა არ უნდა აღემატებოდეს 8-ს.", 3000);
                    return;
                }
                var vehicleData = VehicleManager.GetVehicleToNumber(number);
                if (vehicleData != null)
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"მანქანის ნომერი: {number} | მოდელი: {vehicleData.Model} | მფლობელი: {vehicleData.Holder}", 6000);
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ამოწმებს მანქანის მფლობელს {number} ({vehicleData.Holder})");
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამ სანომრე ნიშნით მანქანა ვერ მოიძებნა", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_setWeather(ExtPlayer player, int weather)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) შეცვალა ამინდი({weather})");
        ...
        public static void CMD_setTime(ExtPlayer player, short hours, short minutes, short seconds)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "არასწორი მონაცემები, საათი 0-23, წუთი 0-59, წამი 0-59", 3000);
                    return;
                }
                Admin.TimeChanged = false;
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) თამაშის დროის დაყენება({hours} {minutes} {seconds})");
                ...
                if (hours != -1 && minutes != -1 && seconds != -1)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენ გაყინული გაქვთ თამაშის დრო {hours}ч. {minutes}м. {seconds}თან. დასაბრუნებლად დააყენეთ -1 -1 -1", 3000);
        ...
        public static void CMD_DelObjects(ExtPlayer player, string locationName)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openDialog", "DelObjects", $"გსურთ წაშალოთ {locationName}?");
        ...
        public static void CMD_teleport(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= 6)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.CantFindPlayerWithId), 3000);
                        Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{player.Name} ({player.Value})შენთან ტელეპორტირებას ცდილობდა (/tp)", 3000);
                        return;
                    }
                }
                Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ტელეპორტირებული  {target.Name} ({target.Value}) სთან", 1, "#636363");
        ...
        public static void CMD_Metpcar(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    if (veh.Value != id) continue;
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ტელეპორტირებული მანქანა თქვენთან {veh.DisplayName} ({id})", 1, "#636363", hideAdminLevel: 9);
        ...
        public static void CMD_tpTocar(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    if (veh.Value != id) continue;
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ტელეპორტირებული მანქანასთან {veh.DisplayName} ({id})", 1, "#636363", hideAdminLevel: 9);
        ...
        public static void CMD_tpcarnumber(ExtPlayer player, string veh_number)
        {
            try
            {
                ...
                foreach (ExtVehicle veh in vehs)
                {
                    ...
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ტელეპორტირებული მანქანასთან {veh.DisplayName} (ID: {veh.Value}; Number: {veh.NumberPlate})", 1, "#636363", hideAdminLevel: 9);
                    NAPI.Entity.SetEntityPosition(player, veh.Position);
                    Trigger.Dimension(player, UpdateData.GetVehicleDimension(veh));
        ...
        public static void CMD_Setdimcar(ExtPlayer player, int id, int newDimensionNumber)
        {
            try
            {
                ...
                {
                    if (veh.Value != id) continue;
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) შეცვალა dimension მანქანისთვის {veh.DisplayName} ({id}) на {newDimensionNumber}.", 1, "#636363", hideAdminLevel: 9);
        ...
        [Command(AdminCommands.Crimeban, "გამოიყენე: /crimeban [ID] [მიზეზი]", GreedyArg = true)]
        public static void BanCrime(ExtPlayer player, int pid, string reason)
        {
            try
            {
                ...
                if (target != null && targetCharacterData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "მოთამაშე უკვე დაბლოკილია დაჭერისა და ბიზვარ სისტემებში.", 5000);
                        return;
                    }

                    Trigger.SendPunishment($"~r~{CommandsAccess.AdminPrefix}{player.Name} შეზღუდული წვდომა კაპტზე და ბიზვარზე. სისტემები პერსონაჟზე {target.Name}({target.Value}) სამუდამოდ. მიზეზი {reason}.", target);
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ამ პირადობის მქონე მოთამაშე ვერ მოიძებნა", 5000);
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.Uncrimeban, "გამოიყენე: /uncrimeban [ID]")]
        public static void Uncrimeban(ExtPlayer player, int pid)
        {
            try
            {
                ...
                if (target != null && targetCharacterData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "მოთამაშე არ არის დაბლოკილი.capture , bizwar.", 5000);
                        return;
                    }

                    Trigger.SendToAdmins(1, $"~r~[A] {player.Name} ({player.Value}) განბლოკილი პერსონაჟი capture ,bizwar  {target.Name} ({target.Value})");
                    Trigger.SendChatMessage(target, $"ადმინისტრატორმა {player.Name} მოგხსნათ ბანი. capture, bizwar.");
        ...
        [Command(AdminCommands.Offcrimeban, "გამოიყენე: /offcrimeban [სახელი_გვარი] [მიზეზი]", GreedyArg = true)]
                ...
                else
                {
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "პერსონაჟი უკვე დაბლოკილია.capture , bizwar.", 3000);
                                return;
                            }

                            Trigger.SendPunishment(
                                $"~r~{CommandsAccess.AdminPrefix}{sessionData.Name} შეზღუდული წვდომა capture , bizwar  {name} სამუდამოდ. მიზეზი: {reason}.");
        ...
        [Command(AdminCommands.Offuncrimeban, "გამოიყენე: /offuncrimeban [სახელი_გვარი]")]
                ...
                else
                {
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            if (!character.IsBannedCrime)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "პერსონაჟი არ არის დაბლოკილი სისტემებში capture, bizwar.", 3000);
                                return;
                            }
        ...
        public static void CMD_flipveh(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    if (veh.Value != id) continue;
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) მანქანა ამოატრიალა {veh.DisplayName} ({id})");
        ...
        public static void CMD_LoadIPL(ExtPlayer player, string ipl)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "თქვენ ატვირთეთ IPL: " + ipl);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_StartEffect(ExtPlayer player, string effect, int dur = 0, bool loop = false)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "თქვენ ჩართეთ Effect: " + effect);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_StopEffect(ExtPlayer player, string effect)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "შენ გამორთე Effect: " + effect);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_offlineJailTarget(ExtPlayer player, string targetName, int time, string reason)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "მოთამაშე იყო ონლაინ, ამიტომ offjail შეიცვალა", 3000);
                    return;
                }
                if (time < 5 || time > 10080)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"თქვენ არ შეგიძლიათ ციხეში 10080 წუთზე მეტი ", 3000);
                    return;
                }
                if (Main.stringGlobalBlock.Any(c => reason.Contains(c)))
                {
                    Trigger.SendToAdmins(1, $"{ChatColors.Red}[A] {player.Name} ({player.Value}) სისტემამ მოხსნა დასჯის მიზეზით: {reason}");
                    Character.BindConfig.Repository.DeleteAdmin(player);
                    return;
                }
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} ჩასვა დემორგანში {targetName}  offline  {time} {deTimeMsg}. მიზეზი: {reason}");
        ...
        public static void CMD_Offunjail(ExtPlayer player, string targetName)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "მოთამაშე იყო ონლაინ, ამიტომ offfunjail შეიცვალა unjail-ით", 3000);
                    return;
                }
                int targetUuid = Main.PlayerUUIDs[targetName];
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} დემორგანისგან გათავისუფლებული {targetName.Replace('_', ' ')}  offline.");
        ...
        public static void CMD_offlineWarnTarget(ExtPlayer player, string targetName, string reason)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "მოთამაშე იყო ონლაინ, ამიტომ offwarn შეიცვალა გაფრთხილებით", 3000);
                    return;
                }
                if (Main.stringGlobalBlock.Any(c => reason.Contains(c)))
                {
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        {
                            Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {sessionData.Name} ({sessionData.Value}) ცადა ბანის დადება {targetName} (offline), რომელსაც აქვს უფრო მაღალი ადმინისტრატორის დონე..");
                            return;
                        }
                        ...
                        if (memberFractionData != null)
                        {
                            Fractions.Table.Logs.Repository.AddOffLogs(memberFractionData.Id, targetName, targetUuid,
                                FractionLogsType.UnInvite, "გაფრთხილება მიიღო");
                        ...
                        Trigger.SetMainTask(() =>
                        {
                            ...
                            Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{sessionData.Name} გაფრთხილება გასცა მოთამაშეს {targetName} offline | {warns}/3. მიზეზი: {reason}");
        ...
        public static void CMD_pritonbanTarget(ExtPlayer player, int id)
        {
            try
            {
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} მოთამაშე მოშორდა {targetPlayer.Name}({targetPlayer.Id})");
        ...
        public static void CMD_offredbaks(ExtPlayer player, string name, int amount)
        {
            try
            {
                ...
                if (target != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "მოთამაშე იყო ონლაინ, ამიტომ offgivereds შეიცვალა გაცემულით", 3000);
                    return;
                }
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"თქვენ წარმატებით მიეცით {amount} StreetCoin მოთამაშეს - {name}", 3000);
        ...
        public static void CMD_ALog(ExtPlayer player)
        {
            try
            {
                ...
                if (adminConfig.ALog)
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Admin Log ჩართულია", 3000);
                else
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Admin Log გამორთლია", 3000);
        ...
        public static void CMD_ELog(ExtPlayer player)
        {
            try
            {
                ...
                if (adminConfig.ELog)
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Error Log ჩართულია", 3000);
                else
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Error Log გამორთლია", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_WinLog(ExtPlayer player)
        {
            try
            {
                ...
                if (adminConfig.WinLog)
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Roulette Win Log ჩართულია", 3000);
                else
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Roulette Win Log გამორთლია", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_NickHistory(ExtPlayer player, int id, bool uuid = false)
        {
            try
            {
                ...
                if (DateTime.Now < sessionData.TimingsData.NextNHistory)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"თქვენ არ შეგიძლიათ გამოიყენოთ ბრძანება 5 წამში ერთხელ.", 3000);
                    return;
                }
                    ...
                    if (targetSessionData != null && targetCharacterData != null)
                    {
                        Trigger.SetTask(() =>
                        {
                            try
                            {
                                ...
                                if (result == null || result.Rows.Count == 0)
                                {
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "მოთამაშეს არასოდეს შეუცვლია მეტსახელი", 3000);
                                    return;
                                }
                                else
                                {
                                    Admin.AdminLog(characterData.AdminLVL, $"{sessionData.Name} ({sessionData.Value}) ამოწმებს სახელის ცვლილებების ისტორიას {targetSessionData.Name} ({targetSessionData.Value})");
                                    sessionData.TimingsData.NextNHistory = DateTime.Now.AddSeconds(5);
                    ...
                    if (Main.PlayerNames.ContainsKey(id))
                    {
                        Trigger.SetTask(() =>
                        {
                            try
                            {
                                ...
                                if (result == null || result.Rows.Count == 0)
                                {
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "მოთამაშეს ან არასოდეს შეუცვლია მეტსახელი, ან ასეთი პასპორტის ნომერი არ არსებობს", 3000);
                                    return;
                                }
                                else
                                {
                                    Admin.AdminLog(characterData.AdminLVL, $"{sessionData.Name} ({sessionData.Value}) ამოწმებს UUID-ის მიერ სახელის ცვლილებების ისტორიას {id}");
                                    sessionData.TimingsData.NextNHistory = DateTime.Now.AddSeconds(5);
        ...
        public static void LeaveFractionOrg(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (index == 0)
                {
                    ...
                    Manager.sendFractionMessage(memberFractionData.Id, "!{#FF8C00}[F] " + $"{player.Name} ({player.Value}) საკუთარი სურვილით გადადგა.", true);
        ...
        [Command("givearmylic")]
        public static void CMD_giveArmylic(ExtPlayer player, int id)
        {
            try
            {
                FractionCommands.giveArmyicalLic(player, Main.GetPlayerByID(id));
            }
            catch (Exception e)
            {
                Log.Write($"CMD_giveArmylic Exception: {e.ToString()}");
            }
        }
        ...
        public static void CMD_checkPrisonTime(ExtPlayer player)
        {
            try
            {
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                if (characterData.ArrestTime >= 1) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"თქვენ დაგრჩათ {Convert.ToInt32(characterData.ArrestTime / 60.0)} წუთი", 3000);
                else if (characterData.DemorganTime >= 1)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"სასჯელი იწურება {Convert.ToInt32(characterData.DemorganTime / 60.0)} წუთი", 5000);
                    if (!characterData.DemorganInfo.Admin.Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"თქვენ ჩაგსვესვ {characterData.DemorganInfo.Admin} მიზეზი: {characterData.DemorganInfo.Reason}", 10000);
                }
                else if (characterData.Warns >= 1)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"ანგარიშზე გაქვთ {characterData.Warns} გაფთხილება", 5000);
                    if (!characterData.WarnInfo.Admin[0].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"გაფრთხილება (1/3) გაცემულია {characterData.WarnInfo.Admin[0]} მიზეზი: {characterData.WarnInfo.Reason[0]}", 10000);
                    if (characterData.Warns >= 2 && !characterData.WarnInfo.Admin[1].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"გაფრთხილება (2/3) გაცემულია {characterData.WarnInfo.Admin[1]} მიზეზი: {characterData.WarnInfo.Reason[1]}", 10000);
                    if (characterData.Warns >= 3 && !characterData.WarnInfo.Admin[2].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"გაფრთხილება (3/3) გაცემულია {characterData.WarnInfo.Admin[2]} მიზეზი: {characterData.WarnInfo.Reason[2]}", 10000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_pcheckPrisonTime(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (targetCharacterData.Unmute > 0)
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"მოთამაშეს {target.Name} დარჩა {targetCharacterData.Unmute / 60} წუთი", 3000);

                if (targetCharacterData.ArrestTime >= 1)
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"მოთამაშეს {target.Name} დარჩა {Convert.ToInt32(targetCharacterData.ArrestTime / 60.0)} წუთი", 3000);

                if (targetCharacterData.DemorganTime >= 1)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"სასჯელი {target.Name} ვადა ამოიწურება {Convert.ToInt32(targetCharacterData.DemorganTime / 60.0)} წუთი", 5000);
                    if (!targetCharacterData.DemorganInfo.Admin.Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"ის ჩასვეს {targetCharacterData.DemorganInfo.Admin} მიზეზი: {targetCharacterData.DemorganInfo.Reason}", 10000);
                }

                if (targetCharacterData.Warns >= 1)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"ანგარიშზე {target.Name} ამ მომენტში {targetCharacterData.Warns} გაფთხილება", 5000);
                    if (!targetCharacterData.WarnInfo.Admin[0].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"გაფრთხილება (1/3) გაცემულია {targetCharacterData.WarnInfo.Admin[0]} მიზეზი: {targetCharacterData.WarnInfo.Reason[0]}", 10000);
                    if (targetCharacterData.Warns >= 2 && !targetCharacterData.WarnInfo.Admin[1].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"გაფრთხილება (2/3) გაცემულია {targetCharacterData.WarnInfo.Admin[1]} მიზეზი: {targetCharacterData.WarnInfo.Reason[1]}", 10000);
                    if (targetCharacterData.Warns >= 3 && !targetCharacterData.WarnInfo.Admin[2].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"გაფრთხილება (3/3) გაცემულია {targetCharacterData.WarnInfo.Admin[2]} მიზეზი: {targetCharacterData.WarnInfo.Reason[2]}", 10000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_gov(ExtPlayer player, string msg)
        {
            try
            {
                ...
                if (Main.stringGlobalBlock.Any(c => testmsg.Contains(c)))
                {
                    ...
                    Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} ({player.Value}) სცადა მიწერა სახელმწიფო კომუნიკაციებისთვის: {msg}");
                ...
                NAPI.Chat.SendChatMessageToAll($"~y~[სახელმწიფო. ახალი ამბები] {player.Name}: {msg}");
        ...
        public static void CMD_go(ExtPlayer player, int sender_id)
        {
            try
            {
                ...
                if (memberFractionData.Id >= 6 && memberFractionData.Id <= 9 || memberFractionData.Id == (int)Fractions.Models.Fractions.ARMY || memberFractionData.Id == (int)Fractions.Models.Fractions.LSNEWS || memberFractionData.Id == (int)Fractions.Models.Fractions.SHERIFF)
                {
                    ...
                    string numberInfo = characterData.Sim == -1 ? "" : $" საკონტაქტო ტელეფონი: {characterData.Sim}";
        ...
        public static void CreateRefCode(ExtPlayer player)
        {
            try
            {
                ...
                string message = "!{#636363}[A] " + $"მოთამაშემ {player.Name} ({player.Value}) შექმნა რეფ.კოდი: " + code;
        ...
        public static void CMD_action(ExtPlayer player, string msg)
        {
            try
            {
                ...
                string message = "!{#636363}[A] " + $"მოთამაშე {player.Name} ({player.Value}) დაწერა в action: " + msg;
                Trigger.SendToAdmins(1, message);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_chatRoll(ExtPlayer player, int first, int second)
        {
            try
            {
                ...
                if (first < 0 || second < 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"ორივე ნომრის მინიმალური მნიშვნელობა - 0.", 3000);
                    return;
                }
                if (first >= 65536 || second >= 65536)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"რიცხვების მაქსიმალური მნიშვნელობა - 65535.", 3000);
                    return;
                }
                if (second < first)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"მეორე რიცხვი პირველზე მეტი უნდა იყოს.", 3000);
                    return;
                }
            ...
            try
            {
                ...
                switch (result)
                {
                    ...
                    case 2:
                        ...
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "try", "!{#E066FF}{name} " + message + " | !{#277C6B}" + " წარმატებით", new int[] { sender.Value });
                    ...
                    default:
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f,
                                     UpdateData.GetPlayerDimension(sender)))
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "try", "!{#E066FF}{name} " + message + " | !{#FF0707}" + " წარუმატებლად", new int[] { sender.Value });
        ...
        private static void Roll(ExtPlayer sender, int first, int second)
        {
            try
            {
                ...
                foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, UpdateData.GetPlayerDimension(sender)))
                    Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "roll", "!{#FF8C00}{name} გადაგდო ნომერი " + result + " | !{#e69500}" + first + " - " + second, new int[] { sender.Value });
            }
            catch (Exception e)
            {
                ...
                switch (cmd)
                {
                    ...
                    case "todo":
                        if (message.IndexOf('*') >= 0)
                        {
                            ...
                            if (msg.Length >= 1 && action.Length >= 1)
                            {
                                ...
                                foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, UpdateData.GetPlayerDimension(sender)))
                                    Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "todo", msg + ",!{#E066FF} - თქვა" + genderCh + " {name}, " + action, names);
                            }
                        }
                        return;
                    case "do":
                    ...
                    case "s":
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 30f,
                                     UpdateData.GetPlayerDimension(sender)))
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "s", "{name} ყვირის: " + message, names);
                    ...
                    case "m":
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 120f, UpdateData.GetPlayerDimension(sender)))
                                Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "m", "!{#FFA500}[მეგაფონი] {name}: " + message, names);
                        }
                        return;
                    case "t":
                        ...
                        foreach (ExtPlayer foreachPlayer in NeptuneEvo.Character.Repository.GetPlayers())
                        {
                            ...
                            {
                                if (foreachSessionData.WorkData.OnWork && foreachPlayer.IsInVehicle) Trigger.SendChatMessage(foreachPlayer, $"~y~[სატვირთო მანქანებსი რაცია] [{sender.Name}]: {message}");
                            }
                        }
                        return;
                    case "w":
                        ...
                        {
                            Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"მოთამაშე ძალიან შორს არის.", 3000);
                            return;
                        }
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, UpdateData.GetPlayerDimension(sender)))
                        {
                            var foreachCharacterData = foreachPlayer.GetCharacterData();
                            if (foreachCharacterData == null) continue;
                            if (foreachPlayer.IsCharacterData() && foreachCharacterData.AdminLVL >= 1) Trigger.SendChatMessage(foreachPlayer, "!{#C5C7C7}" + $"[ჩურჩული] {sender.Name} ({sender.Value}): {message}");
                            else if (foreachPlayer != sender && foreachPlayer != target) Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "w", "!{#C5C7C7}{name} ჩაიჩურჩულა რაღაც {name}", names);
                        }
                        Trigger.SendChatMessage(sender, "!{#C5C7C7}" + $"[ჩურჩული] {sender.Name.Replace('_', ' ')} ({sender.Value}): {message}");
                        ...
                        if (targetFriends.ContainsKey(sender.Name))
                        {
                            ...
                            {
                                Trigger.SendChatMessage(target, "!{#C5C7C7}" + $"[ჩურჩული] {sender.Name.Split('_')[0]} ({sender.Value}): {message}");
                            }
                            else
                            {
                                Trigger.SendChatMessage(target, "!{#C5C7C7}" + $"[ჩურჩული] {sender.Name.Replace('_', ' ')} ({sender.Value}): {message}");
                            }
                        }
                        else Trigger.SendChatMessage(target, "!{#C5C7C7}" + $"[ჩურჩული] " + (characterData.Gender ? "უცხო ადამიანი" : "უცხო ადამიანი") + $" ({sender.Value}): {message}");
